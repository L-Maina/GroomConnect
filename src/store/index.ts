    ...
    (set, get) => ({
      ...
      login: (user, token) => set({ 
        user: {
          ...
          activeMode: user.activeMode || getDefaultMode(user),
        }, 
        token, 
...
function getDefaultMode(user: User): UserMode {
  // Admin always stays in ADMIN mode
  if (user.roles?.includes('ADMIN') || user.role === 'ADMIN') return 'ADMIN';
  // Business owner defaults to PROVIDER mode
  if (user.roles?.includes('BUSINESS_OWNER') || user.role === 'BUSINESS_OWNER') return 'PROVIDER';
  ...
  persist(
    (set, get) => ({
      ...
      setCountry: (countryCode) => {
        ...
        const dialCode = countryToDialCode[countryCode] || '+1';
      ...
      detectLocation: async () => {
        ...
        try {
          ...
          let countryCode = timezoneToCountry[timezone] || 'US';
...
export function getCountryInfo(countryCode: string) {
  return {
    ...
    dialCode: countryToDialCode[countryCode] || '+1',
...
export const useBusinessDataStore = create<BusinessDataState>()(
  persist(
    ...
    {
      name: 'groomconnect-business-data',
    }
  )
);
