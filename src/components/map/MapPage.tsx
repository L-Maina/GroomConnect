...
export const MapPage: React.FC<MapPageProps> = ({ businesses, onSelectBusiness, onBookBusiness }) => {
  ...
  // Map center and zoom - Default to Nairobi, Kenya (GroomConnect HQ)
    ...
    if (typeof navigator !== 'undefined' && navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          sessionStorage.setItem('groomconnect-location-permission', 'granted');
        },
        (error) => {
          setIsLocating(false);
          ...
          if (error.code === error.PERMISSION_DENIED) {
            sessionStorage.setItem('groomconnect-location-permission', 'denied');
          }
        },
        { 
  ...
  return (
    <div className="h-[calc(100vh-64px)] relative bg-gray-100 dark:bg-slate-900">
      ...
      {/* Map Theme Switcher - Liquid Glass morphism */}
      <div className="absolute right-4 top-36 z-[999]">
        <div className="relative">
          <button 
            onClick={() => setShowThemeMenu(!showThemeMenu)} 
            className="w-11 h-11 backdrop-blur-2xl bg-white/70 rounded-xl shadow-lg shadow-black/5 flex items-center justify-center hover:bg-white/80 transition-colors border border-white/30"
          >
            <Layers className="h-5 w-5 text-gray-600" />
          </button>
          
          {/* Theme Menu - Liquid Glass morphism */}
          <AnimatePresence>
            {showThemeMenu && (
              <motion.div 
                ...
                exit={{ opacity: 0, x: 10 }}
                className="absolute right-12 top-0 backdrop-blur-2xl bg-white/70 rounded-xl shadow-2xl shadow-black/5 border border-white/30 overflow-hidden w-36"
              >
                {Object.entries(mapThemes).map(([key, theme]) => (
                  <button 
                    key={key} 
                    onClick={() => { 
                      setMapTheme(key as MapTheme); 
                      setShowThemeMenu(false); 
                    }} 
                    className={cn(
                      "w-full px-4 py-2.5 text-left text-sm hover:bg-white/50 transition-colors",
                      mapTheme === key && "bg-blue-50/50 text-[#007AFF] font-medium"
                    )}
                  >
                    {theme.name}
                  </button>
