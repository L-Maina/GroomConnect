...
import type { User, Service, Staff, Booking } from '@/types';
...
const initialServices: Service[] = [
  { id: 's1', businessId: 'b1', name: 'Classic Haircut', description: 'Traditional cut with modern finish', category: 'Haircuts & Styling', duration: 30, price: 35, isActive: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 's2', businessId: 'b1', name: 'Beard Trim', description: 'Expert beard shaping and grooming', category: 'Beard Grooming', duration: 20, price: 20, isActive: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 's3', businessId: 'b1', name: 'Hot Towel Shave', description: 'Luxury straight razor shave', category: 'Beard Grooming', duration: 30, price: 45, isActive: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 's4', businessId: 'b1', name: 'Hair Coloring', description: 'Full color transformation', category: 'Hair Coloring', duration: 120, price: 150, isActive: true, createdAt: new Date(), updatedAt: new Date() },
];

const initialStaff: Staff[] = [
  { id: 'st1', businessId: 'b1', name: 'Marcus Johnson', role: 'Master Barber', bio: '15 years of experience in classic and modern cuts', isActive: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 'st2', businessId: 'b1', name: 'David Chen', role: 'Style Specialist', bio: 'Award-winning stylist specializing in trends', isActive: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 'st3', businessId: 'b1', name: 'Lisa Park', role: 'Color Expert', bio: 'Certified colorist with 8 years experience', isActive: true, createdAt: new Date(), updatedAt: new Date() },
];

const initialBookings: Booking[] = [
  { id: 'b1', customerId: 'c1', businessId: 'b1', serviceId: 's1', staffId: 'st1', date: '2024-01-20', startTime: '10:00', endTime: '10:30', status: 'CONFIRMED', totalAmount: 35, createdAt: new Date(), updatedAt: new Date() },
  { id: 'b2', customerId: 'c2', businessId: 'b1', serviceId: 's4', staffId: 'st3', date: '2024-01-20', startTime: '11:30', endTime: '13:30', status: 'CONFIRMED', totalAmount: 150, createdAt: new Date(), updatedAt: new Date() },
  { id: 'b3', customerId: 'c3', businessId: 'b1', serviceId: 's2', staffId: 'st1', date: '2024-01-20', startTime: '14:00', endTime: '14:20', status: 'PENDING', totalAmount: 20, createdAt: new Date(), updatedAt: new Date() },
  { id: 'b4', customerId: 'c4', businessId: 'b1', serviceId: 's1', staffId: 'st2', date: '2024-01-21', startTime: '09:00', endTime: '09:30', status: 'CONFIRMED', totalAmount: 35, createdAt: new Date(), updatedAt: new Date() },
  { id: 'b5', customerId: 'c5', businessId: 'b1', serviceId: 's3', staffId: 'st1', date: '2024-01-21', startTime: '10:00', endTime: '10:30', status: 'COMPLETED', totalAmount: 45, createdAt: new Date(), updatedAt: new Date() },
];
...
export const BusinessDashboard: React.FC<BusinessDashboardProps> = ({
      ...
      if (editingService) {
        setServices(prev => prev.map(s => 
          s.id === editingService.id 
            ? { ...s, ...serviceForm, updatedAt: new Date() }
            : s
        ));
        showSaveMessage('success', 'Service updated successfully!');
      } else {
        ...
        showSaveMessage('success', 'Service added successfully!');
      }
  ...
  const handleDeleteService = (serviceId: string) => {
    if (confirm('Are you sure you want to delete this service?')) {
      ...
      showSaveMessage('success', 'Service deleted successfully!');
    }
  };
  ...
  const handleToggleService = (serviceId: string) => {
    ...
    showSaveMessage('success', 'Service status updated!');
  };
  ...
  const handleUpdateBookingStatus = (bookingId: string, status: Booking['status']) => {
    ...
    showSaveMessage('success', `Booking ${status.toLowerCase()}!`);
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full gradient-bg flex items-center justify-center text-white font-medium">
                              {customerNames[booking.customerId]?.[0] || 'U'}
                            </div>
                            <div>
                              <div className="font-medium">{customerNames[booking.customerId] || 'Customer'}</div>
                              <div className="text-sm text-muted-foreground">{getServiceName(booking.serviceId)}</div>
                            </div>
                          </div>
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <h3 className="font-semibold">{customerNames[booking.customerId] || 'Customer'}</h3>
                                <p className="text-sm text-muted-foreground">
                                  {getServiceName(booking.serviceId)} • {getStaffName(booking.staffId)}
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-4">
                        ...
                        <div key={booking.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">{customerNames[booking.customerId]}</p>
                            </div>
                          </div>
                          <div className="text-right">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/50">
                  <div className="w-14 h-14 rounded-full gradient-bg flex items-center justify-center text-white font-bold text-lg">
                    {customerNames[selectedBooking.customerId]?.[0] || 'U'}
                  </div>
                  <div>
                    <div className="font-semibold">{customerNames[selectedBooking.customerId] || 'Customer'}</div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{getServiceName(selectedBooking.serviceId)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Staff</p>
                    <p className="font-medium">{getStaffName(selectedBooking.staffId)}</p>
                  </div>
                  <div>
