...
export const LocationPermissionPopup: React.FC<LocationPermissionPopupProps> = ({
  ...
  const requestLocation = async () => {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (countryCode && countryToCurrency[countryCode]) {
            ...
            localStorage.setItem('groomconnect-location-permission', 'granted');
            localStorage.setItem('groomconnect-location', JSON.stringify({
              lat: latitude,
              lng: longitude,
              country: countryCode,
              currency: countryInfo.currency,
      ...
      if (timezoneCountry && countryToCurrency[timezoneCountry]) {
        ...
        localStorage.setItem('groomconnect-location-permission', 'denied');
        onPermissionDenied?.();
      } else if (err.code === 2) {
  ...
  // Handle skip
  const handleSkip = () => {
    localStorage.setItem('groomconnect-location-permission', 'skipped');
    onClose(false);
  };
...
export const useLocationPermission = () => {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('groomconnect-location-permission');
    const locationData = localStorage.getItem('groomconnect-location');
  ...
  return {
    ...
    setPermissionAsked: (state: PermissionState | 'skipped') => {
      localStorage.setItem('groomconnect-location-permission', state);
      setHasAsked(true);
