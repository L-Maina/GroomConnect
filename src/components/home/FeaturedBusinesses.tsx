  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 bg-gradient-to-br from-primary/20 to-secondary/20">
        ...
        {/* Favorite Button - Show for guests and client mode users */}
        {canShowFavorite && (
          <button
            onClick={handleFavoriteClick}
            className="absolute top-3 right-3 h-9 w-9 rounded-full glass flex items-center justify-center transition-all hover:scale-110 hover:shadow-lg"
          >
            <Heart
              className={cn(
                'h-5 w-5 transition-colors',
                isFavorite && !isGuest ? 'fill-red-500 text-red-500' : 'text-foreground/70 group-hover:text-red-400'
              )}
            />
          </button>
        )}
