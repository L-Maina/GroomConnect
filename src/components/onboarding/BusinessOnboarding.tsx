...
export const BusinessOnboarding: React.FC<BusinessOnboardingProps> = ({
  ...
  // Check if already submitted - must be defined before using in useState
  ...
  // Business details
  const [businessName, setBusinessName] = useState(user?.businessName || existingApplication?.businessName || '');
  const [businessDescription, setBusinessDescription] = useState(existingApplication?.businessDescription || '');
  const [businessAddress, setBusinessAddress] = useState(existingApplication?.businessAddress || '');
  const [businessCity, setBusinessCity] = useState(existingApplication?.businessCity || '');
  const [businessCountry, setBusinessCountry] = useState(existingApplication?.businessCountry || '');
  const [businessPhone, setBusinessPhone] = useState(existingApplication?.businessPhone || '');
  const [businessEmail, setBusinessEmail] = useState(existingApplication?.businessEmail || '');
  const [businessWebsite, setBusinessWebsite] = useState(existingApplication?.businessWebsite || '');
  
  // ID Verification - locked if already submitted
  const [idType, setIdType] = useState<'PASSPORT' | 'NATIONAL_ID' | 'DRIVERS_LICENSE'>(
    existingApplication?.idType || user?.idType || 'PASSPORT'
  );
  const [idNumber, setIdNumber] = useState(existingApplication?.idNumber || user?.idNumber || '');
  const [idDocument, setIdDocument] = useState<File | null>(null);
  const [idDocumentPreview, setIdDocumentPreview] = useState<string | null>(
    existingApplication?.idDocumentUrl || user?.idDocumentUrl || null
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const application = {
        ...
        idType,
        idNumber,
        idDocumentUrl: idDocumentPreview || undefined,
        businessName,
        businessDescription,
        businessAddress,
        businessCity,
      ...
      localStorage.setItem('groomconnect-verification-status', 'PENDING');
      localStorage.setItem('groomconnect-business-name', businessName);
            ...
            {isVerified ? (
              <>
                ...
                <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/20 text-left mb-4">
                  ...
                  <p className="text-sm text-muted-foreground mt-2 mb-1">Approved On</p>
                  <p className="font-medium text-green-500">
                    {new Date(existingApplication?.reviewedAt || Date.now()).toLocaleDateString()}
                  </p>
                </div>
                <GlassButton
                  variant="primary"
              ...
              <>
                ...
                <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20 text-left mb-4">
                  ...
                  <p className="font-medium text-red-500">{existingApplication?.rejectionReason || 'No reason provided'}</p>
                  <p className="text-sm text-muted-foreground mt-2 mb-1">Reviewed On</p>
              ...
              <>
                ...
                <div className="p-4 rounded-lg bg-muted/50 text-left">
                  ...
                  <p className="text-sm text-muted-foreground mt-2 mb-1">Submitted</p>
                  <p className="font-medium">
                    {new Date(existingApplication?.submittedAt || Date.now()).toLocaleDateString()}
                  </p>
                </div>
              </>
            )}
                  ...
                  <div>
                    ...
                    <LocationAutocomplete
                      ...
                      onChange={(value, details) => {
                        ...
                        if (details?.city) setBusinessCity(details.city);
                        if (details?.country) setBusinessCountry(details.country);
                      }}
